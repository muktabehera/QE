...
* TC-42732 - Pathfinder_Edge_Device_Profiles POST:

  Verify that VideoEdge device profile is not getting created with invalid (mulitcastaddress)  using POST /devices/veProfiles  .
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanEdit': False,
   ...
   'id': 'No_proxy_InvalidMulticast',
   'manifestRequestFrequency': 600,
   'maximumDownloadTime': 54000,
   'name': 'No_proxy_InvalidMulticast',
   ...
   'rtspService': {'multicastAddress': '10.10.10.1000',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42732')
    @pytest.mark.Pathfinder_Edge_Device_Profiles
    @pytest.mark.POST
    def test_TC_42732_POST_Pathfinder_Edge_Device_Profiles_Unable_To_Create_With_Invalid_Multicast_Address(self, context):
        """TC-42732 - Pathfinder_Edge_Device_Profiles-POST
           Verify that VideoEdge device profile is not getting created with invalid (mulitcastaddress)  using POST /devices/veProfiles  ."""
        # Define a test step
        with pytest.allure.step("""Verify that VideoEdge device profile is not getting created with invalid (mulitcastaddress)  using POST /devices/veProfiles  ."""):
            ...
            videoEdgeDevicePropertyDetails = context.sc.VideoEdgeDevicePropertyDetails(
                ...
                rtspService={
                    ...
                    'rtspStreamingPort': 554,
                    'multicastEnabled': True,
                    'multicastAddress': '10.10.10.1000',
                    'multicastTtl': 5,
                    'unicastFailoverEnabled': True
                },
                tokenExpirationTime=180,
