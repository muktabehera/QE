...
* TC-42726 - Pathfinder_Edge_Device_Profiles POST:

  Verify that user is able to Create (AllowAll Configurations ,prepositioning & token,description, enable rtsp &http) VideoEdge device profile using POST /devices/veProfiles  .
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42726')
    @pytest.mark.Pathfinder_Edge_Device_Profiles
    @pytest.mark.POST
    def test_TC_42726_POST_Pathfinder_Edge_Device_Profiles_Create_All_Configurations_Prepositioning_Token_With_Rtspand_Http(self, context):
        """TC-42726 - Pathfinder_Edge_Device_Profiles-POST
           Verify that user is able to Create (AllowAll Configurations ,prepositioning & token,description, enable rtsp &http) VideoEdge device profile using POST /devices/veProfiles  ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to Create (AllowAll Configurations ,prepositioning & token,description, enable rtsp &http) VideoEdge device profile using POST /devices/veProfiles  ."""):
