...
* TC-42614 - Pathfinder_Edge_Device_Profiles PATCH:

  Verify that user is able to modify(PATCH)[visible AllConfigurations,enable token,multicast disabled,rtsp server properties,http server,enable prepostions] VideoEdge Profile details using VideoEdge profile ID for request "/devices/veProfiles/{id}" .
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42614')
    @pytest.mark.Pathfinder_Edge_Device_Profiles
    @pytest.mark.PATCH
    def test_TC_42614_PATCH_Pathfinder_Edge_Device_Profiles_Able_To_Modify_All_Configurations_Rtsp_Http_Preposition_Token_Enabled_And_Multicast_Disabled(self, context):
        """TC-42614 - Pathfinder_Edge_Device_Profiles-PATCH
           Verify that user is able to modify(PATCH)[visible AllConfigurations,enable token,multicast disabled,rtsp server properties,http server,enable prepostions] VideoEdge Profile details using VideoEdge profile ID for request "/devices/veProfiles/{id}" ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to modify(PATCH)[visible AllConfigurations,enable token,multicast disabled,rtsp server properties,http server,enable prepostions] VideoEdge Profile details using VideoEdge profile ID for request "/devices/veProfiles/{id}" ."""):
