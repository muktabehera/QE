...
* TC-42611 - Pathfinder_Edge_Device_Profiles PATCH:

  Verify that user is able to modify(PATCH)[visible AllConfigurations,enable prepositioing enabled,multicast enabled,http server properties] VideoEdge Profile details using VideoEdge profile ID for request "/devices/veProfiles/{id}" .
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42611')
    @pytest.mark.Pathfinder_Edge_Device_Profiles
    @pytest.mark.PATCH
    def test_TC_42611_PATCH_Pathfinder_Edge_Device_Profiles_Able_To_Modify_With_Prepositioning_Configurations_Multicast_Http_Enabled(self, context):
        """TC-42611 - Pathfinder_Edge_Device_Profiles-PATCH
           Verify that user is able to modify(PATCH)[visible AllConfigurations,enable prepositioing enabled,multicast enabled,http server properties] VideoEdge Profile details using VideoEdge profile ID for request "/devices/veProfiles/{id}" ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to modify(PATCH)[visible AllConfigurations,enable prepositioing enabled,multicast enabled,http server properties] VideoEdge Profile details using VideoEdge profile ID for request "/devices/veProfiles/{id}" ."""):
