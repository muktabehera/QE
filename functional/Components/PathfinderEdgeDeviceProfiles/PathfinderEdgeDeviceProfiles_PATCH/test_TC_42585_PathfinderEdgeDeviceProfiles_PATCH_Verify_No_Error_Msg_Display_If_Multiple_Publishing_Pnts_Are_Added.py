...
* TC-42585 - Pathfinder_Edge_Device_Profiles PATCH:

  Verify that no error message is displayed if user is able to add [multiple publishing points]  using VideoEdge profile ID for request PATCH "/devices/veProfiles/{id}" .
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanEdit': True,
   ...
   'httpService': {'httpPort': 80,
                   ...
                   'protocol': 'http',
                   'serviceActive': True,
                   'vodPublishingPoints': [{'name': 'testUpdate2',
                                            'originId': '12Client'},
                                           {'name': 'testUpdate1',
                                            'originId': 'originFTP'}]},
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42585')
    @pytest.mark.Pathfinder_Edge_Device_Profiles
    @pytest.mark.PATCH
    def test_TC_42585_PATCH_Pathfinder_Edge_Device_Profiles_Verify_No_Error_Message_Displayed_If_Multiple_Publishing_Points_Are_Added(self, context):
        """TC-42585 - Pathfinder_Edge_Device_Profiles-PATCH
           Verify that no error message is displayed if user is able to add [multiple publishing points]  using VideoEdge profile ID for request PATCH "/devices/veProfiles/{id}" ."""
        # Define a test step
        with pytest.allure.step("""Verify that no error message is displayed if user is able to add [multiple publishing points]  using VideoEdge profile ID for request PATCH "/devices/veProfiles/{id}" ."""):
