...
* TC-45600 - Configurations PATCH:

  Verify that user is unable to Modify[host] with invalid value( on providing space) for configuration using request PATCH "/configurations/{id} " .
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanCreate': True, 'host': ' '}
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-45600')
    @pytest.mark.Configurations
    @pytest.mark.PATCH
    def test_TC_45600_PATCH_Configurations_Host_With_Space(self, context):
        """TC-45600 - Configurations-PATCH
           Verify that user is unable to Modify[host] with invalid value( on providing space) for configuration using request PATCH "/configurations/{id} " ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to Modify[host] with invalid value( on providing space) for configuration using request PATCH "/configurations/{id} " ."""):
