...
* TC-45602 - Configurations PATCH:

  Verify that user is able to Update [configAdminCanCreate= false] for Configuration using request PATCH /configurations/{id} ".
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanCreate': False,
   'host': 'PATCH_Configurations_configAdminCanCreateIsFalse'}
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-45602')
    @pytest.mark.Configurations
    @pytest.mark.PATCH
    def test_TC_45602_PATCH_Configurations_Config_Admin_Can_Create_Is_False(self, context):
        """TC-45602 - Configurations-PATCH
           Verify that user is able to Update [configAdminCanCreate= false] for Configuration using request PATCH /configurations/{id} "."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to Update [configAdminCanCreate= false] for Configuration using request PATCH /configurations/{id} "."""):
