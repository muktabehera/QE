...
* TC-44063 - Origins PATCH:

  Verify that user is able to modify origin on providing all roles(HTTP, FTP Download, FTP Secure Download, FSA) in parameter 'baseUris' using request PATCH '/origins'.
...
JSON data sent to PathFinder in this test:
  ...
  {'baseUris': [{'roles': ['common.filesystemfetch'],
   ...
   'name': 'Updated Origin with All Rules',
   'tokenGenerator': None,
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44063')
    @pytest.mark.Origins
    @pytest.mark.PATCH
    def test_TC_44063_PATCH_Origins_All_Rules(self, context):
        """TC-44063 - Origins-PATCH
           Verify that user is able to modify origin on providing all roles(HTTP, FTP Download, FTP Secure Download, FSA) in parameter 'baseUris' using request PATCH '/origins'."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to modify origin on providing all roles(HTTP, FTP Download, FTP Secure Download, FSA) in parameter 'baseUris' using request PATCH '/origins'."""):
