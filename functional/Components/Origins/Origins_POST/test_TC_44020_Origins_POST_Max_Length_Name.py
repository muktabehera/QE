...
* TC-44020 - Origins POST:

  Verify that user is able to create origin on providing max length of 100 character in parameter 'Name' using request POST '/origins'.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44020')
    @pytest.mark.Origins
    @pytest.mark.POST
    def test_TC_44020_POST_Origins_Max_Length_Name(self, context):
        """TC-44020 - Origins-POST
           Verify that user is able to create origin on providing max length of 100 character in parameter 'Name' using request POST '/origins'."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to create origin on providing max length of 100 character in parameter 'Name' using request POST '/origins'."""):
