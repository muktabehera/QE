...
* TC-46707 - Diagnosis POST:

  Verify that all the subsequent network rules are displayed as 'Unevaluated' in audience after any network rule is 'Failed' as per the client machine using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46707')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46707_POST_Diagnosis_Audiences_Network_Rule_Unevaluated(self, context):
        """TC-46707 - Diagnosis-POST
           Verify that all the subsequent network rules are displayed as 'Unevaluated' in audience after any network rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that all the subsequent network rules are displayed as 'Unevaluated' in audience after any network rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""):
