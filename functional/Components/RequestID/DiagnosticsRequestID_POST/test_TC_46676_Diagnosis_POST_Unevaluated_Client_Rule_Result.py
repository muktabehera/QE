...
* TC-46676 - Diagnosis POST:

  Verify that all the subsequent client rules are  displayed as 'Unevaluated' after any client rule is 'Failed' as per the client machine using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46676')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46676_POST_Diagnosis_Unevaluated_Client_Rule_Result(self, context):
        """TC-46676 - Diagnosis-POST
           Verify that all the subsequent client rules are  displayed as 'Unevaluated' after any client rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that all the subsequent client rules are  displayed as 'Unevaluated' after any client rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""):
