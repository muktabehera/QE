...
* TC-46696 - Diagnosis POST:

  Verify that all the network locations rules are displayed with result(Succeeded, Failed, Unevaluated) as per the client machine using request POST  /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46696')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46696_POST_Diagnosis_All_Network_Results(self, context):
        """TC-46696 - Diagnosis-POST
           Verify that all the network locations rules are displayed with result(Succeeded, Failed, Unevaluated) as per the client machine using request POST  /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that all the network locations rules are displayed with result(Succeeded, Failed, Unevaluated) as per the client machine using request POST  /diagnostices/requestId."""):
