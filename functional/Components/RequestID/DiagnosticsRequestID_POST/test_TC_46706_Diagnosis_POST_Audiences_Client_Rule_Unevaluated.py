...
* TC-46706 - Diagnosis POST:

  Verify that all the subsequent client rules are displayed as 'Unevaluated' in audience after any client rule is 'Failed' as per the client machine using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46706')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46706_POST_Diagnosis_Audiences_Client_Rule_Unevaluated(self, context):
        """TC-46706 - Diagnosis-POST
           Verify that all the subsequent client rules are displayed as 'Unevaluated' in audience after any client rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that all the subsequent client rules are displayed as 'Unevaluated' in audience after any client rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""):
