...
* TC-46697 - Diagnosis POST:

  Verify that all the subsequent network locations rules are  displayed as 'Unevaluated' after any network locations rule is 'Failed' as per the client machine using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46697')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46697_POST_Diagnosis_Unevaluated_Network_Rule_Result(self, context):
        """TC-46697 - Diagnosis-POST
           Verify that all the subsequent network locations rules are  displayed as 'Unevaluated' after any network locations rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that all the subsequent network locations rules are  displayed as 'Unevaluated' after any network locations rule is 'Failed' as per the client machine using request POST /diagnostices/requestId."""):
