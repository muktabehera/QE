...
* TC-46710 - Diagnosis POST:

  Verify that client result is displayed as 'Failed' in audience if any the groups result 'Failed' using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46710')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46710_POST_Diagnosis_Audiences_Any_Client_Group_Failed(self, context):
        """TC-46710 - Diagnosis-POST
           Verify that client result is displayed as 'Failed' in audience if any the groups result 'Failed' using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that client result is displayed as 'Failed' in audience if any the groups result 'Failed' using request POST /diagnostices/requestId."""):
