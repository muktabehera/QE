...
* TC-46711 - Diagnosis POST:

  Verify that network location result is displayed as 'Failed' in audience if both the groups result 'Failed' using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46711')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46711_POST_Diagnosis_Audiences_Both_Network_Group_Failed(self, context):
        """TC-46711 - Diagnosis-POST
           Verify that network location result is displayed as 'Failed' in audience if both the groups result 'Failed' using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that network location result is displayed as 'Failed' in audience if both the groups result 'Failed' using request POST /diagnostices/requestId."""):
