...
* TC-46698 - Diagnosis POST:

  Verify that network locations result is displayed as 'Failed' if both the groups result 'Failed' using request POST /diagnostices/requestId.
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-46698')
    @pytest.mark.Diagnosis
    @pytest.mark.POST
    def test_TC_46698_POST_Diagnosis_Both_Network_Rule_Failed(self, context):
        """TC-46698 - Diagnosis-POST
           Verify that network locations result is displayed as 'Failed' if both the groups result 'Failed' using request POST /diagnostices/requestId."""
        # Define a test step
        with pytest.allure.step("""Verify that network locations result is displayed as 'Failed' if both the groups result 'Failed' using request POST /diagnostices/requestId."""):
