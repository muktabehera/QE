...
* TC-42651 - Pathfinder_Edge_Device POST:

  Verify that correct messgae is displayed if user provide the wrong profile name in "deviceProfileId"  for POST request  /devices/veDevices .
...
JSON data sent to PathFinder in this test:
  ...
  {'alertCount': 0,
   ...
   'deviceProfileId': 'Invalid',
   'edgeDeviceRoles': ['ORIGIN', 'EDGE', 'DISTRIBUTION'],
   'id': 'POST_veDevices_ProfileNull',
   'multicastHost': 'POST_veDevices',
   'name': 'POST_veDevices_ProfileNull',
   'online': False,
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42651')
    @pytest.mark.Pathfinder_Edge_Device
    @pytest.mark.POST
    def test_TC_42651_POST_Pathfinder_Edge_Device_Verify_Correct_Message_Displaye_With_Wrong_Profile_Id(self, context):
        """TC-42651 - Pathfinder_Edge_Device-POST
           Verify that correct messgae is displayed if user provide the wrong profile name in "deviceProfileId"  for POST request  /devices/veDevices ."""
        # Define a test step
        with pytest.allure.step("""Verify that correct messgae is displayed if user provide the wrong profile name in "deviceProfileId"  for POST request  /devices/veDevices ."""):
