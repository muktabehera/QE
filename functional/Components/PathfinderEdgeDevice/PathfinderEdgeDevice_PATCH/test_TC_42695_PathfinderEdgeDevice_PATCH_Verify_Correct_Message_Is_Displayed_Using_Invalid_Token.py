...
* TC-42695 - Pathfinder_Edge_Device PATCH:

  Verify that correct message is displayed if user hit the API PATCH "/devices/veDevices/{id}" with incorrect token.


Equivalent test CURL command:

  curl -H "Host: <client_host>" -H "Authorization: Bearer incorrecttoken"
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42695')
    @pytest.mark.Pathfinder_Edge_Device
    @pytest.mark.PATCH
    def test_TC_42695_PATCH_Pathfinder_Edge_Device_Verify_Correct_Message_Is_Displayed_Using_Invalid_Token(self, context):
        """TC-42695 - Pathfinder_Edge_Device-PATCH
           Verify that correct message is displayed if user hit the API PATCH "/devices/veDevices/{id}" with incorrect token."""
        # Define a test step
        with pytest.allure.step("""Verify that correct message is displayed if user hit the API PATCH "/devices/veDevices/{id}" with incorrect token."""):
