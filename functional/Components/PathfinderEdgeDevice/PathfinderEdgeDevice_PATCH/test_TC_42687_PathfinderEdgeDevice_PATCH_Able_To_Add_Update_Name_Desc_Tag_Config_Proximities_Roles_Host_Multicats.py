...
* TC-42687 - Pathfinder_Edge_Device PATCH:

  Verify that user is able to update (name, desc,tag,add config,add proximities,add roles,host settings,multicast settings) videoEdge device using PATCH "/devices/veDevices/{id}".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-42687')
    @pytest.mark.Pathfinder_Edge_Device
    @pytest.mark.PATCH
    def test_TC_42687_PATCH_Pathfinder_Edge_Device_Able_To_Add_Update_Name_Desc_Tag_Config_Proximities_Roles_Host_Multicats(self, context):
        """TC-42687 - Pathfinder_Edge_Device-PATCH
           Verify that user is able to update (name, desc,tag,add config,add proximities,add roles,host settings,multicast settings) videoEdge device using PATCH "/devices/veDevices/{id}"."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to update (name, desc,tag,add config,add proximities,add roles,host settings,multicast settings) videoEdge device using PATCH "/devices/veDevices/{id}"."""):
