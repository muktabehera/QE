...
* TC-45527 - Pathfinder_Edge_Global_Configuration PATCH:

  Verify that user is unable to update deliveryServicePort on providing value greater than 65535 characters for VideoEdge Global configuration using request PATCH /devices/veGlobalConfig .
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-45527')
    @pytest.mark.Pathfinder_Edge_Global_Configuration
    @pytest.mark.PATCH
    def test_TC_45527_PATCH_Pathfinder_Edge_Global_Configuration_Max_Length_Exceeds_Del_Service_Port(self, context):
        """TC-45527 - Pathfinder_Edge_Global_Configuration-PATCH
           Verify that user is unable to update deliveryServicePort on providing value greater than 65535 characters for VideoEdge Global configuration using request PATCH /devices/veGlobalConfig ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to update deliveryServicePort on providing value greater than 65535 characters for VideoEdge Global configuration using request PATCH /devices/veGlobalConfig ."""):
