...
* TC-43356 - Authorization_Systems PATCH:

  Verify that user is able to update [Add Configurations] while "visibleInAllConfigurations: false  for Authorization System using request PATCH authorizationSystems/{id}  ".
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanEdit': True,
   ...
   'name': 'PATCH_authorizationSystem_addConfigurations',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43356')
    @pytest.mark.Authorization_Systems
    @pytest.mark.PATCH
    def test_TC_43356_PATCH_Authorization_Systems_Add_Configurations(self, context):
        """TC-43356 - Authorization_Systems-PATCH
           Verify that user is able to update [Add Configurations] while "visibleInAllConfigurations: false  for Authorization System using request PATCH authorizationSystems/{id}  "."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to update [Add Configurations] while "visibleInAllConfigurations: false  for Authorization System using request PATCH authorizationSystems/{id}  "."""):
