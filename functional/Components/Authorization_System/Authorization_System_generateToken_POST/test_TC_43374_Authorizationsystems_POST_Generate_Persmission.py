...
* TC-43374 - Authorization_Systems POST:

  Verify that user is able to generate Token with permission [Generate] while same is selected in the Authorization system using request POST /authorizationSystems/{id}/generateToken  ".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43374')
    @pytest.mark.Authorization_Systems
    @pytest.mark.POST
    def test_TC_43374_POST_Authorization_Systems_Generate_Persmission(self, context):
        """TC-43374 - Authorization_Systems-POST
           Verify that user is able to generate Token with permission [Generate] while same is selected in the Authorization system using request POST /authorizationSystems/{id}/generateToken  "."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to generate Token with permission [Generate] while same is selected in the Authorization system using request POST /authorizationSystems/{id}/generateToken  "."""):
