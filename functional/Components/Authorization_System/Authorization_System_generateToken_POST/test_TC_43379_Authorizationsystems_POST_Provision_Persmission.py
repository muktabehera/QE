...
* TC-43379 - Authorization_Systems POST:

  Verify that user is able to generate Token with permission [Provision] while same is selected in the Authorization system using request POST /authorizationSystems/{id}/generateToken  ".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43379')
    @pytest.mark.Authorization_Systems
    @pytest.mark.POST
    def test_TC_43379_POST_Authorization_Systems_Provision_Persmission(self, context):
        """TC-43379 - Authorization_Systems-POST
           Verify that user is able to generate Token with permission [Provision] while same is selected in the Authorization system using request POST /authorizationSystems/{id}/generateToken  "."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to generate Token with permission [Provision] while same is selected in the Authorization system using request POST /authorizationSystems/{id}/generateToken  "."""):
