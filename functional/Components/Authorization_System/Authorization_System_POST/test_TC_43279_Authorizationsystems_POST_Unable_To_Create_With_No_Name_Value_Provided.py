...
* TC-43279 - Authorization_Systems POST:

  Verify that user is unable to create Authorization System if "Name" parameter is not provided using request POST /authorizationSystems.
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanEdit': True,
   ...
   'grantablePermissions': ['manageSystem',
                            ...
                            'generate',
                            'manageConfiguration'],
   'id': 'POST_authorizationSystem_UnableToCreate_withNoNameValueProvided',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43279')
    @pytest.mark.Authorization_Systems
    @pytest.mark.POST
    def test_TC_43279_POST_Authorization_Systems_Unable_To_Create_With_No_Name_Value_Provided(self, context):
        """TC-43279 - Authorization_Systems-POST
           Verify that user is unable to create Authorization System if "Name" parameter is not provided using request POST /authorizationSystems."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to create Authorization System if "Name" parameter is not provided using request POST /authorizationSystems."""):
