...
* TC-43333 - Authorization_Systems POST:

  Verify that user is able to create Authorization System with multiple configurations using request POST /authorizationSystems.
...
JSON data sent to PathFinder in this test:
  ...
  {'configAdminCanEdit': True,
   ...
   'grantablePermissions': ['manageSystem',
                            ...
                            'generate',
                            'manageConfiguration'],
   'id': 'POST_authorizationSystem_withMultipleConfigurations',
   'macKey': '12345689713265487923156234497813265489712654897',
   'name': 'POST_authorizationSystem_withMultipleConfigurations',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43333')
    @pytest.mark.Authorization_Systems
    @pytest.mark.POST
    def test_TC_43333_POST_Authorization_Systems_With_Multiple_Configurations(self, context):
        """TC-43333 - Authorization_Systems-POST
           Verify that user is able to create Authorization System with multiple configurations using request POST /authorizationSystems."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to create Authorization System with multiple configurations using request POST /authorizationSystems."""):
