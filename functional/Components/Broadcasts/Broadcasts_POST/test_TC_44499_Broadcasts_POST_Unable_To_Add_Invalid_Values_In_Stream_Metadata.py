...
* TC-44499 - Broadcasts POST:

  Verify that user is unable to add invalid values in streamMetaData using request POST "/broadcasts".
...
JSON data sent to PathFinder in this test:
  ...
  {'id': 'BroadcastPOST_StreamMetaData',
   'name': 'BroadcastPOST_StreamMetaData',
   'streams': [{'connectionPoints': [{'maxConnections': 10000,
                                      'mode': 'PULL',
                                      'url': 'http://media.blacktrash.org/stsp.m3u8'}],
                'id': 'streheham',
                'streamMetadata': {'mimeType': 'videomp4'},
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44499')
    @pytest.mark.Broadcasts
    @pytest.mark.POST
    def test_TC_44499_POST_Broadcasts_Unable_To_Add_Invalid_Values_In_Stream_Metadata(self, context):
        """TC-44499 - Broadcasts-POST
           Verify that user is unable to add invalid values in streamMetaData using request POST "/broadcasts"."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to add invalid values in streamMetaData using request POST "/broadcasts"."""):
