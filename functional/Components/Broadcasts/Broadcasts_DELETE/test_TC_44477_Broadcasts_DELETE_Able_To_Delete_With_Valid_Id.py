...
* TC-44477 - Broadcasts DELETE:

  Verify that user is able to DELETE the broadcast with valid id using request "/broadcasts/{id}".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44477')
    @pytest.mark.Broadcasts
    @pytest.mark.DELETE
    def test_TC_44477_DELETE_Broadcasts_Able_To_Delete_With_Valid_Id(self, context):
        """TC-44477 - Broadcasts-DELETE
           Verify that user is able to DELETE the broadcast with valid id using request "/broadcasts/{id}"."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to DELETE the broadcast with valid id using request "/broadcasts/{id}"."""):
