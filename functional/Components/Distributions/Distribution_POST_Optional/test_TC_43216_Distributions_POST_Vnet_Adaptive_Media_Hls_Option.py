...
* TC-43216 - Distributions POST:

  Verify that user is able to send adaptive streaming media file(HLS) for distribution using request POST "/distributions" with VideoNet.
  ...
  {'activationDate': '2017-09-20T07:36:46.542Z',
   ...
   'files': [{'auxiliaryFiles': [{'relativeUrl': 'qumu_4e322c4a016e4f698783f006065468df_HLS_time1_1.m3u8'},
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'width': 10}}],
   'id': 'vnetAdaptiveHLSOpt',
   'name': 'Distribution with VNET HLS Opt',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43216')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_43216_POST_Distributions_Vnet_Adaptive_Media_Hls(self, context):
        """TC-43216 - Distributions-POST
           Verify that user is able to send adaptive streaming media file(HLS) for distribution using request POST "/distributions" with VideoNet."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to send adaptive streaming media file(HLS) for distribution using request POST "/distributions" with VideoNet."""):
