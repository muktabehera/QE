...
* TC-43680 - Distributions POST:

  Verify that user is able to send slides(PPT, PPTx) for distribution using request POST "/distributions" with standalone VNE as public delivery system.
...
JSON data sent to PathFinder in this test:
  ...
  {'activationDate': '2017-09-20T07:36:46.542Z',
   ...
   'files': [{'id': 'pptOpt',
              'sourceUrl': 'qedorigin://Auto_storage/art-slide.ppt',
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'width': 10}}],
   'id': 'slidePPTOpt',
   'name': 'Distribution with PPT Slide Opt',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43680')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_43680_POST_Distributions_Distributions_Vne_Ppt_Slide(self, context):
        """TC-43680 - Distributions-POST
           Verify that user is able to send slides(PPT, PPTx) for distribution using request POST "/distributions" with standalone VNE as public delivery system."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to send slides(PPT, PPTx) for distribution using request POST "/distributions" with standalone VNE as public delivery system."""):
