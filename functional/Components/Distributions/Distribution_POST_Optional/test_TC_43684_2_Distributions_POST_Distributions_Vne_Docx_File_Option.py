...
* TC-43684 - Distributions POST:

  Verify that user is able to send document(doc, docx,htm) for distribution using request POST "/distributions" with standalone VNE as public delivery system.
  ...
  {'activationDate': '2017-09-20T07:36:46.542Z',
   ...
   'files': [{'id': 'docxopt',
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'width': 10}}],
   'id': 'vneDocxFileOpt',
   'name': 'Distribution with VNE DOCX File Opt',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43684')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_43684_POST_Distributions_Distributions_Vne_Docx_File(self, context):
        """TC-43684 - Distributions-POST
           Verify that user is able to send document(doc, docx,htm) for distribution using request POST "/distributions" with standalone VNE as public delivery system."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to send document(doc, docx,htm) for distribution using request POST "/distributions" with standalone VNE as public delivery system."""):
