...
* TC-43213 - Distributions POST:

  Verify that user is able to send adaptive streaming media file(HLS) for distribution using request POST "/distributions" with VNE.
...
JSON data sent to PathFinder in this test:
  ...
  {'activationDate': '2017-09-20T07:36:46.542Z',
   ...
   'files': [{'auxiliaryFiles': [{'relativeUrl': 'qumu_4e322c4a016e4f698783f006065468df_HLS_time1_1.m3u8'},
              ...
              'id': 'HLS2opt',
              'sourceUrl': 'qedorigin://Auto_storage/HLS/qumu_4e322c4a016e4f698783f006065468df_HLS_time1_Master.m3u8',
              'streamMetadata': {'bitrateKbps': 100,
                                 'contentType': 'UNSPECIFIED',
                                 'height': 5,
                                 'mimeType': 'video/mp4',
                                 'width': 10}}],
   'id': 'adaptiveStreamHLSOpt',
   'name': 'Distribution with Vne HLS Opt',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43213')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_43213_POST_Distributions_Vne_Adaptive_Media_Hls(self, context):
        """TC-43213 - Distributions-POST
           Verify that user is able to send adaptive streaming media file(HLS) for distribution using request POST "/distributions" with VNE."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to send adaptive streaming media file(HLS) for distribution using request POST "/distributions" with VNE."""):
            ...
            distributionDetails = context.sc.DistributionDetails(
                ...
                files=[{
                    ...
                    'HLS2opt',
                    'sourceUrl':
                    'qedorigin://Auto_storage/HLS/qumu_4e322c4a016e4f698783f006065468df_HLS_time1_Master.m3u8',
                    'auxiliaryFiles': [{
                        'relativeUrl':
