...
* TC-43668 - Distributions POST:

  Verify that user is able to send image file for distribution using request POST "/distributions" with standalone VNE.
  ...
  {'activationDate': '2017-09-20T07:36:46.542Z',
   ...
   'files': [{'id': 'imageOpt',
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'width': 10}}],
   'id': 'vneImageOpt',
   'name': 'Distribution with vne image opt',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43668')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_43668_POST_Distributions_Vne_Image(self, context):
        """TC-43668 - Distributions-POST
           Verify that user is able to send image file for distribution using request POST "/distributions" with standalone VNE."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to send image file for distribution using request POST "/distributions" with standalone VNE."""):
