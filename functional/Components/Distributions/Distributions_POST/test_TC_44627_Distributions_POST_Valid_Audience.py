...
* TC-44627 - Distributions POST:

  Verify that user is able to create Distribution  if valid  targetAudiences 'ID' value is provided using request POST /distributions .
  ...
  {'activationDate': '2017-09-06T07:36:46.542Z',
   ...
   'files': [{'id': 'FileID123',
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'mimeType': 'Video/mp4',
                                 'width': 10}}],
   'id': 'validDistributionAudi5',
   'name': 'Distribution with Valid Audience5',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44627')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_44627_POST_Distributions_Valid_Audience(self, context):
        """TC-44627 - Distributions-POST
           Verify that user is able to create Distribution  if valid  targetAudiences 'ID' value is provided using request POST /distributions ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to create Distribution  if valid  targetAudiences 'ID' value is provided using request POST /distributions ."""):
