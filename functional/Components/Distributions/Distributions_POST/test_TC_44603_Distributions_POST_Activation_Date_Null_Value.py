...
* TC-44603 - Distributions POST:

  Verify that user is unable to create Distribution  if  'activationDate' parameter is not provided using request POST /distributions .
  ...
  {'activationDate': '',
   ...
   'files': [{'id': 'sample123',
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'mimeType': 'Video/mp4',
                                 'width': 10}}],
   'id': 'ActivationDate_NullValue',
   'name': 'ActivationDate_NullValue',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44603')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_44603_POST_Distributions_Activation_Date_Null_Value(self, context):
        """TC-44603 - Distributions-POST
           Verify that user is unable to create Distribution  if  'activationDate' parameter is not provided using request POST /distributions ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to create Distribution  if  'activationDate' parameter is not provided using request POST /distributions ."""):
