...
* TC-44610 - Distributions POST:

  Verify that user is unable to create Distribution  if invalid value for  'distributionPolicy' parameter is provided using request POST /distributions .
...
JSON data sent to PathFinder in this test:
  ...
  {'activationDate': '2017-09-06T07:36:46.542Z',
   'distributionPolicy': 'invalid_value',
   'files': [{'id': 'sample123',
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'mimeType': 'Video/mp4',
                                 'width': 10}}],
   'id': 'distributionPolicy_invalidValue',
   'name': 'distributionPolicy_invalidValue',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44610')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_44610_POST_Distributions_Invalid_Value(self, context):
        """TC-44610 - Distributions-POST
           Verify that user is unable to create Distribution  if invalid value for  'distributionPolicy' parameter is provided using request POST /distributions ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to create Distribution  if invalid value for  'distributionPolicy' parameter is provided using request POST /distributions ."""):
