...
* TC-43758 - Distributions POST:

  Verify that user is able to send multiple files(flv,mp4,mp3) for distribution using request POST "/distributions" with VNE as public delivery service.
   ...
   'files': [{'id': 'mp3',
             ...
             {'id': 'wma',
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'mimeType': 'video/mp4',
                                 'width': 10}}],
   'id': 'vneMultipleFiles1',
   'name': 'Distribution with VNE FLV MP4 MP3',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43758')
    @pytest.mark.Distributions
    @pytest.mark.POST
    def test_TC_43758_POST_Distributions_Vne_Flv_Mp4_Mp3(self, context):
        """TC-43758 - Distributions-POST
           Verify that user is able to send multiple files(flv,mp4,mp3) for distribution using request POST "/distributions" with VNE as public delivery service."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to send multiple files(flv,mp4,mp3) for distribution using request POST "/distributions" with VNE as public delivery service."""):
