...
* TC-43778 - Distributions PATCH:

  Verify that correct message is displayed on if distribution id doesnt exist using request PATCH "/distributions/{Id}" .
...
Equivalent test CURL command:
  ...
  curl -H "Host: <client_host>" -H "Authorization: Bearer <valid_token>"
       ...
       "<PF_host>://<client_host>/distributions/IdnotExists"
  ...
  {'activationDate': '2017-09-04T07:36:46.542Z',
   ...
   'files': [{'auxiliaryFiles': [{'relativeUrl': 'qumu_4e322c4a016e4f698783f006065468df_HLS_time1_1.m3u8'},
              ...
              'streamMetadata': {'bitrateKbps': 100,
                                 ...
                                 'width': 10}}],
   'name': 'Updated Distribution with protected VNE Audience',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43778')
    @pytest.mark.Distributions
    @pytest.mark.PATCH
    def test_TC_43778_PATCH_Distributions_Id_Not_Exists(self, context):
        """TC-43778 - Distributions-PATCH
           Verify that correct message is displayed on if distribution id doesnt exist using request PATCH "/distributions/{Id}" ."""
        # Define a test step
        with pytest.allure.step("""Verify that correct message is displayed on if distribution id doesnt exist using request PATCH "/distributions/{Id}" ."""):
