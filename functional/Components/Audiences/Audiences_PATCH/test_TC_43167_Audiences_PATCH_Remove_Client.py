...
* TC-43167 - Audiences PATCH:

  Verify that user is able to update (Remove Client) Audience using PATCH "audiences/{id}".
...
JSON data sent to PathFinder in this test:
  ...
  {'clients': [{'id': 'clientsWithAllDetails'}],
   ...
   'name': 'PATCH_AudienceModifiedRemoveClient',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43167')
    @pytest.mark.Audiences
    @pytest.mark.PATCH
    def test_TC_43167_PATCH_Audiences_Remove_Client(self, context):
        """TC-43167 - Audiences-PATCH
           Verify that user is able to update (Remove Client) Audience using PATCH "audiences/{id}"."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to update (Remove Client) Audience using PATCH "audiences/{id}"."""):
