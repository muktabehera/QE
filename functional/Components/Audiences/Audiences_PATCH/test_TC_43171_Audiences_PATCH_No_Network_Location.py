...
* TC-43171 - Audiences PATCH:

   Verify that user is able to modify audience while removing mandatory field (Network Location) from Audience using PATCH "audiences/{id}".
...
JSON data sent to PathFinder in this test:
  ...
  {'clients': [{'id': 'clientsWithAllDetails'}],
   ...
   'name': 'PATCH_audience_noNetworkLocation',
   'networkLocations': []}
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43171')
    @pytest.mark.Audiences
    @pytest.mark.PATCH
    def test_TC_43171_PATCH_Audiences_No_Network_Location(self, context):
        """TC-43171 - Audiences-PATCH
            Verify that user is able to modify audience while removing mandatory field (Network Location) from Audience using PATCH "audiences/{id}"."""
        # Define a test step
        with pytest.allure.step(""" Verify that user is able to modify audience while removing mandatory field (Network Location) from Audience using PATCH "audiences/{id}"."""):
