...
* TC-43163 - Audiences PATCH:

  Verify that 'id' parameter is mandatory for request  PATCH audiences/{id} ".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43163')
    @pytest.mark.Audiences
    @pytest.mark.PATCH
    def test_TC_43163_PATCH_Audiences_Id_Mandatory(self, context):
        """TC-43163 - Audiences-PATCH
           Verify that 'id' parameter is mandatory for request  PATCH audiences/{id} "."""
        # Define a test step
        with pytest.allure.step("""Verify that 'id' parameter is mandatory for request  PATCH audiences/{id} "."""):
