...
* TC-43166 - Audiences PATCH:

  Verify that user is able to update (Add Client) Audience using PATCH "audiences/{id}".
...
JSON data sent to PathFinder in this test:
  ...
  {'clients': [{'id': 'clientsWithAllDetails'}, {'id': 'autoclient'}],
   ...
   'name': 'PATCH_AudienceModifiedClient',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43166')
    @pytest.mark.Audiences
    @pytest.mark.PATCH
    def test_TC_43166_PATCH_Audiences_Add_Client(self, context):
        """TC-43166 - Audiences-PATCH
           Verify that user is able to update (Add Client) Audience using PATCH "audiences/{id}"."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to update (Add Client) Audience using PATCH "audiences/{id}"."""):
