...
* TC-43154 - Audiences POST:

  Verify that user is able to create Audience with Multiple Clients, Network Locations and Delivery Systems using request POST /audiences.
...
JSON data sent to PathFinder in this test:
  ...
  {'clients': [{'id': 'clientsWithAllDetails'}, {'id': 'autoclient'}],
   ...
   'id': 'POST_audience_MultipleClient_DelSystem_NwLoc',
   'name': 'POST_audience_MultipleClient_DelSystem_NwLoc',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43154')
    @pytest.mark.Audiences
    @pytest.mark.POST
    def test_TC_43154_POST_Audiences_Multiple_Client_Del_System_Nw_Loc(self, context):
        """TC-43154 - Audiences-POST
           Verify that user is able to create Audience with Multiple Clients, Network Locations and Delivery Systems using request POST /audiences."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to create Audience with Multiple Clients, Network Locations and Delivery Systems using request POST /audiences."""):
