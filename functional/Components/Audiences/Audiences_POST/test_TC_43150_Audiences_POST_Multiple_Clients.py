...
* TC-43150 - Audiences POST:

  Verify that user is able to create Audience with Multiple Clients using request POST /audiences.
...
JSON data sent to PathFinder in this test:
  ...
  {'clients': [{'id': 'clientsWithAllDetails'}, {'id': 'autoclient'}],
   ...
   'id': 'POSTAudiMultipleCleint',
   'name': 'POSTAudiMultipleCleint',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43150')
    @pytest.mark.Audiences
    @pytest.mark.POST
    def test_TC_43150_POST_Audiences_Multiple_Clients(self, context):
        """TC-43150 - Audiences-POST
           Verify that user is able to create Audience with Multiple Clients using request POST /audiences."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to create Audience with Multiple Clients using request POST /audiences."""):
