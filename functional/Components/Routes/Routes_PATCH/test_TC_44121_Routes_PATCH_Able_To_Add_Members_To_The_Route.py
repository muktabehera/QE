...
* TC-44121 - Routes PATCH:

  Verify that user is able to Update(add members) route having root-5 hops->10 hops each from root device using request PATCH "/routes".
  ...
  {'configAdminCanEdit': True,
   ...
   'hops': [{'hops': [{'hops': [],
            ...
            {'hops': [{'hops': [],
             ...
             'memberType': 'EDGE_DEVICE'}],
   'id': '55devices',
   'modificationDate': '2016-03-09T12:26:52Z',
   'name': 'devices55',
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44121')
    @pytest.mark.Routes
    @pytest.mark.PATCH
    def test_TC_44121_PATCH_Routes_Able_To_Add_Members_To_The_Route(self, context):
        """TC-44121 - Routes-PATCH
           Verify that user is able to Update(add members) route having root-5 hops->10 hops each from root device using request PATCH "/routes"."""
        # Define a test step
        with pytest.allure.step("""Verify that user is able to Update(add members) route having root-5 hops->10 hops each from root device using request PATCH "/routes"."""):
