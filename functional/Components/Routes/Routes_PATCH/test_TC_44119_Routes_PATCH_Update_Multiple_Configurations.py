...
* TC-44119 - Routes PATCH:

  Verify that user is able to add multiple system configurations in existing route using request PATCH "/routes".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-44119')
    @pytest.mark.Routes
    @pytest.mark.PATCH
    def test_TC_44119_PATCH_Routes_Update_Multiple_Configurations(self, context):
        """TC-44119 - Routes-PATCH
           Verify that user is able to add multiple system configurations in existing route using request PATCH "/routes"."""
        # Define a test step
        with pytest.allure.step("""First create route without configurations using request POST "/routes"."""):
        ...
        # Define a test step
        with pytest.allure.step("""Then verify that user is able to add multiple system configurations in existing route using request PATCH "/routes"."""):
