...
* TC-43870 - Proximity_Zones PATCH:

  Verify that user is unable to Modify [Proximity Zone] with empty values in  'Ranges' for Proximity Zone using request PATCH /proximities{id} ".
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43870')
    @pytest.mark.Proximity_Zones
    @pytest.mark.PATCH
    def test_TC_43870_PATCH_Proximity_Zones_Unable_To_Modify_With_Empty_Values_In_Ranges(self, context):
        """TC-43870 - Proximity_Zones-PATCH
           Verify that user is unable to Modify [Proximity Zone] with empty values in  'Ranges' for Proximity Zone using request PATCH /proximities{id} "."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to Modify [Proximity Zone] with empty values in  'Ranges' for Proximity Zone using request PATCH /proximities{id} "."""):
