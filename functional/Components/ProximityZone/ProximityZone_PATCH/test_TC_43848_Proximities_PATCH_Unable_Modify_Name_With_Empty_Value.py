...
* TC-43848 - Proximity_Zones PATCH:

  Verify that user is unable to Modify[name]  with empty value  for Proximity Zone using request PATCH /proximities{id} " .
...
class Test_PFE_Components(object):
    ...
    @pytest.allure.link('https://jira.qumu.com/browse/TC-43848')
    @pytest.mark.Proximity_Zones
    @pytest.mark.PATCH
    def test_TC_43848_PATCH_Proximity_Zones_Unable_Modify_Name_With_Empty_Value(self, context):
        """TC-43848 - Proximity_Zones-PATCH
           Verify that user is unable to Modify[name]  with empty value  for Proximity Zone using request PATCH /proximities{id} " ."""
        # Define a test step
        with pytest.allure.step("""Verify that user is unable to Modify[name]  with empty value  for Proximity Zone using request PATCH /proximities{id} " ."""):
